...
@router.post(
    "/installations/{installation_id}/alexa/diagnostic/paperino-v2",
    response_class=RedirectResponse,
)
async def diagnostic_paperino_v2(
    installation_id: UUID,
    request: Request,
    context: Annotated[TenantContext, console_context_dependency],
    session: Annotated[AsyncSession, session_dependency],
) -> RedirectResponse:
    """Send the isolated Paperino v2 endpoint to Amazon for diagnosis."""
    _admin(context)
    installation = await _installation(session, context, installation_id)
    values = await _form(request)
    if not _valid_csrf(values.get("csrf_token", ""), request.cookies.get(CSRF_COOKIE), context):
    ...
    accepted = await send_paperino_diagnostic_v2_safely(session, installation, entity)
    session.add(
        AuditEvent(
            tenant_id=context.tenant_id,
            installation_id=installation.id,
